...
public class GolembigEntity extends Monster implements IAnimatable {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(DragonofforestModEntities.GOLEMBIG.get(), 20, 4, 4));
	}

	private final ServerBossEvent bossInfo = new ServerBossEvent(this.getDisplayName(), ServerBossEvent.BossBarColor.RED,
			ServerBossEvent.BossBarOverlay.PROGRESS);

	public GolembigEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonofforestModEntities.GOLEMBIG.get(), world);
	}

	public GolembigEntity(EntityType<GolembigEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setCustomName(new TextComponent("Big Golem"));
	...
	public static void init() {
		SpawnPlacements.register(DragonofforestModEntities.GOLEMBIG.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(GolembigEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
