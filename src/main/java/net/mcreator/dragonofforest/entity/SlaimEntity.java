...
public class SlaimEntity extends Monster implements IAnimatable {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(DragonofforestModEntities.SLAIM.get(), 20, 4, 4));
	}

	public SlaimEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonofforestModEntities.SLAIM.get(), world);
	}

	public SlaimEntity(EntityType<SlaimEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(DragonofforestModEntities.SLAIM.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(SlaimEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
